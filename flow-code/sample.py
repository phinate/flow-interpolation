...
def make_data_hists2d(data_list, feature_name_list, context_name_list, weight_name = None, num_bins = 20, filter_IQR = False, quantiles=(0,0.2)):
    data_hists_all = []
    context_points = []
    bins_x = []
    bins_y = []

    for data in data_list:  # loop over signal points
        df = ak_to_ndarray(data[feature_name_list])
        if filter_IQR:
            Q1 = np.quantile(df, quantiles[0])
            Q3 = np.quantile(df, quantiles[1])
            IQR = Q3 - Q1
            mask = np.any(~((df < (Q1 - 1.5 * IQR)) | (df > (Q3 + 1.5 * IQR))), axis=1)
            df = df[mask]
        x, y = df[:, 0].to_numpy(), df[:, 1].to_numpy()
        if weight_name is not None:
            weights = data[weight_name].to_numpy()
        else:
            weights = None
        ...
        context_points.append([data[0][name] for name in context_name_list])
    ...
    return data_hists_all, bins_x, bins_y, context_points
